import { Avatar, avatarBg } from '@river/ui'
import { useEffect, useState } from 'react'
import { Input } from '@/components/ui/input'
import { Textarea } from '@/components/ui/textarea'
import { deletePersona, resetPersona, restorePersona, savePersona, updateLobby, useRiver, useT, type RiverState } from '@/lib/river'
...
function DeleteButton({ p, confirm, onAsk }: { p: Persona; confirm: boolean; onAsk: () => void }) {
  ...
  return (
    ...
    >
      {confirm ? t.desktop.opponents.confirmDelete : t.desktop.btn.delete}
    </button>
  )
}
...
function Editor({ p, onDone, confirm, onAsk }: { p: Persona; onDone: () => void; confirm: boolean; onAsk: () => void }) {
  ...
  const o = useT().desktop.opponents
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2.5">
        ...
        <Input value={d.ini} placeholder={o.iniPh} onChange={(e) => set({ ini: e.target.value })} className={cn(field, 'w-11 px-1.5 text-center')} />
        <Input value={d.name} placeholder={o.namePh} onChange={(e) => set({ name: e.target.value })} className={cn(field, 'min-w-0 flex-1 font-semibold')} />
        <Input value={d.tag} placeholder={o.tagPh} onChange={(e) => set({ tag: e.target.value })} className={cn(field, 'w-[76px] text-[13px]')} />
      </div>
      <Input value={d.desc} placeholder={o.descPh} onChange={(e) => set({ desc: e.target.value })} className={cn(field, 'text-[13px]')} />
      <div className="flex items-center gap-2">
        <span className="w-[52px] text-xs text-muted-foreground">{o.hue}</span>
        {HUES.map((h) => (
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={done} className="rounded-full bg-foreground px-4 py-[7px] text-[13px] text-white">{o.done}</button>
      </div>
    </div>
  )
}
...
export function Opponents() {
  ...
  const create = async () => {
    ...
    const f = o.fresh
    const p = await savePersona({ name: f.name(n), tag: f.tag, ini: f.ini, hue: HUES[(Math.random() * HUES.length) | 0], desc: '', prompt: f.prompt })
      ...
      <div className="mx-auto flex max-w-[1120px] flex-col gap-5 px-8 pt-10 pb-16">
        <div className="flex items-end gap-4">
          <div className="flex flex-1 flex-col gap-1.5">
            <div className="text-[28px] font-semibold">{t.desktop.nav.opponents}</div>
            <div className="text-[15px] text-pretty text-subtle">{o.intro}</div>
          </div>
          <button onClick={create} className="rounded-full bg-foreground px-[18px] py-2.5 text-sm font-medium whitespace-nowrap text-white">{o.create}</button>
        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex min-w-0 flex-1 flex-col">
                        <div className="flex items-center gap-1.5">
                          ...
                          {!p.builtin && <span className="rounded-[5px] bg-[oklch(0.96_0.02_255)] px-1.5 py-px text-[11px] whitespace-nowrap text-[oklch(0.5_0.15_255)]">{o.custom}</span>}
                        </div>
                        <span className="text-[13px] text-muted-foreground">{p.desc || o.noDesc}</span>
                      </div>
                    </div>
                    <PromptBox p={p} />
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1" />
                      {p.edited && (
                        <button onClick={() => void resetPersona(p.id)} className="text-xs whitespace-nowrap text-muted-foreground hover:text-foreground">{o.reset}</button>
                      )}
                      <button onClick={() => (setEditing(p.id), setConfirm(null))} className={cn(pillBtn, 'border-input bg-white')}>{t.desktop.btn.edit}</button>
