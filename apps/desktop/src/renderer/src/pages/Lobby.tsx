...
import { go, invoke, openRules, startGuided, startTable, updateLobby, useEvent, useRiver, useT } from '@/lib/river'
import { cn } from '@/lib/utils'
import { BLINDS } from '../../../shared/personas'
import type { HandSummary, Lobby as LobbyT } from '../../../shared/types'

// 与字典 lobby.presets 按下标对应；vs 表示描述里要带上这位对手的当前名字
const PRESETS: { cfg: Omit<LobbyT, 'mode'>; vs?: string }[] = [
  { cfg: { size: 3, blinds: 0, picks: ['bai', 'zen'] } },
  { cfg: { size: 6, blinds: 1, picks: ['li', 'prof', 'bai', 'k', 'rock'] } },
  { cfg: { size: 2, blinds: 2, picks: ['li'] }, vs: 'li' }
]
...
function Recent() {
  ...
  return (
    <div className={cn(card, 'flex flex-col gap-2.5 p-[18px]')}>
      <span className="text-[15px] font-semibold">{l.recent}</span>
      {!hands.length && <span className="text-[13px] text-muted-foreground">{l.noRecent}</span>}
      {hands.map((h) => (
      ...
      <div className="mx-auto grid max-w-[1120px] grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)] items-start gap-6 px-8 pt-10 pb-16">
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <div className="text-[28px] font-semibold tracking-[-0.01em]">{l.title}</div>
            <div className="text-[15px] text-subtle">{l.intro}</div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {PRESETS.map((o, i) => {
              const { t: title, d } = l.presets[i]
              const vs = o.vs && personas.find((p) => p.id === o.vs)?.name
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[13px] leading-normal text-muted-foreground">{vs ? `${d} · ${l.vs(vs)}` : d}</span>
                </button>
              )
            })}
          </div>
          <div className="flex flex-col gap-[18px] rounded-2xl border bg-white px-[22px] py-5">
            ...
            <Row label={l.blinds}>
              <div className="flex flex-1 flex-wrap items-center gap-x-4 gap-y-2">
                ...
                />
                <span className="text-[13px] whitespace-nowrap text-muted-foreground">{l.buyIn(buy)}</span>
              </div>
            </Row>
            <Row label={l.opponents} top>
              <div className="flex flex-1 flex-col gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {personas.length < need
                    ? l.fewer(personas.length, personas.length + 1)
                    : lb.picks.length >= need ? l.picked(need) : l.pickedSome(lb.picks.length, need)}
                </span>
              </div>
            </Row>
            <Row label={l.mode}>
              <div className="flex flex-1 flex-wrap items-center gap-x-4 gap-y-2">
                <Segmented
                  value={lb.mode}
                  options={[{ label: l.modes.coach, value: 'coach' }, { label: l.modes.free, value: 'free' }]}
                  onChange={(mode) => updateLobby({ mode })}
                />
                <span className="text-[13px] text-muted-foreground">{l.modeDesc[lb.mode]}</span>
              </div>
            </Row>
        ...
        <div className="flex flex-col gap-4 pt-[70px]">
          {view && (
            <button onClick={() => go('table')} className="flex flex-col gap-1 rounded-[14px] bg-foreground px-[18px] py-4 text-left text-white">
              <span className="text-[15px] font-semibold">{l.back}</span>
              <span className="text-[13px] text-[#a1a1a6]">{view.title} · {t.desktop.handNo(view.handNo)}</span>
            </button>
          )}
          <div className={cn(card, 'flex flex-col gap-2.5 p-[18px]')}>
            <span className="text-[15px] font-semibold">{l.firstTime}</span>
            <span className="text-[13px] leading-[1.6] text-subtle">{l.firstDesc}</span>
            <div className="flex gap-2">
              <button onClick={openRules} className={cn(pillBtn, 'border border-input hover:bg-accent')}>{l.rules}</button>
              <button onClick={() => startGuided()} className={cn(pillBtn, 'bg-foreground text-white hover:bg-foreground/85')}>{l.guided}</button>
            </div>
          </div>
          <Recent />
        </div>
