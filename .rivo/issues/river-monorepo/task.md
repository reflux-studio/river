...
**结果**：
- 已完成，提交 `bdb7aad`。
- 独立审阅见 `reviews/T1-1.md`，结论为可以完成，没有代码缺陷。
- 打包验证：
  - 通过 CDP 读取回放，并写入一次设置后恢复；
  - asar 的内容和迁移前一致，不需要 hoisted；
  - 打包后“真实打一手并写入回放”这一项，放到整体验证中和真实模型联调一起在打包版本上完成（审阅发现 F1）。
- `build.yml` 要到 T9 才修复，在那之前不能发版（F2）。
...
**结果**：
- 已完成：实现提交 `c9eb060`，在 worktree 中开发，已快进合并。
- 独立审阅：`reviews/T7-1.md`，结论为通过。
- 后续补充：
  - F1：补了两条测试，覆盖“开启自动下载”和“后台下载失败不推送”；
  - F5：后台检查的下载失败原来没人处理，会产生未处理的 rejection，改为只写日志，和 error 事件一样静默。先确认去掉修复时测试报错，再修。
- 转入整体验证：F2（打包版“已是最新”和断网提示）、F4（macOS 的 Squirrel 在下载完成之后才失败）。F3 可以接受。
...
3. **中英双语全流程**：
   ...
   - 新用户选英文，再走一遍同样的流程。
   - **模型回复语言**（真实联调）：用 mac 的**打包版本**（这样同时覆盖 T1 审阅 F1：打包后写入手牌记录），接任一真实提供方（例如 DeepSeek），开一桌教练局。中文和英文各跑至少 2 手，检查对手发言、教练讲解、复盘的语言是否和所选语言一致，把截图和摘录记录到 `evidence/overall/`。mock 测试只能证明发出去的提示词没有汉字，证明不了模型会用所选语言回答。
