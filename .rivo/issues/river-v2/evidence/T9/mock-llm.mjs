...
let failOpponent = process.env.FAIL === '1'
...
http.createServer(async (req, res) => {
  ...
  if (tools.includes('act')) {
    ...
    msg = { role: 'assistant', content: null, tool_calls: [{ id: 'call_' + id, type: 'function', function: { name: 'act', arguments: JSON.stringify(actArgs(prompt)) } }] }
    ...
    const text = /底池赔率/.test(prompt) && /user/.test(prompt) && prompt.includes('什么是')
