...
describe('迁移', () => {
  ...
  it('v1 库升级：旧提示词覆盖保留，旧设置字段不再下发', async () => {
    ...
    c.close()
    await db.initDb({ url, ...crypto })
    expect(db.getSettings()).toEqual({ ...{ speed: 1, coachPersona: 0, level: 'novice', hard: false, felt: 'green', feltCustom: '#2f6b55', back: 'red', fx: 'full', models: {} }, speed: 2 })
...
describe('kv', () => {
  it('默认值', async () => {
    expect(db.getSettings()).toEqual({ speed: 1, coachPersona: 0, level: 'novice', hard: false, felt: 'green', feltCustom: '#2f6b55', back: 'red', fx: 'full', models: {} })
  ...
  it('settings 部分更新合并，并持久化、同步缓存', async () => {
    ...
    const want = {
      speed: 2, coachPersona: 0, level: 'novice', hard: true, felt: 'green', feltCustom: '#2f6b55', back: 'red', fx: 'full',
